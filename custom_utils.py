...
def launch_enhanced_refresh_ui(input_df, show_author: bool = True):
    ...
    # def update_labels():
    #     workspace_label.value = f"<b>🗂️ Workspaces</b> ({get_workspace_count()})"
    #     dataset_label.value = f"<b>📅 Semantic Models</b> ({get_dataset_count()})"
    #     tables_label.value = f"<b>𝄜 Tables</b> ({get_tables_count()}/{get_tables_total()}) <span style='color:gray; font-size:12px;'>(Hold Ctrl/Cmd to select multiple)</span>"
    #     partitions_label.value = f"<b>🧩 Partitions</b> ({get_partitions_count()}/{get_partitions_total()}) <span style='color:gray; font-size:12px;'>(Updates based on table selection)</span>"
    ...
    #  App Assembly
    html_content_author = (
    '<div style="font-size:20px;font-weight:bold;padding:10px;color:white;background:linear-gradient(90deg, #e68900, #ff9800);'
    'border-radius:12px;display:flex;justify-content:space-between;align-items:center;">'
    '<span>Enhanced Semantic Model Refresh</span>'
    '<a href="https://www.linkedin.com/in/jai-rathinavel/" target="_blank" style="color:white;text-decoration:none;display:flex;'
    'align-items:center;font-size:16px;font-weight:normal;">'
    '<svg xmlns="http://www.w3.org/2000/svg" height="20px" width="20px" viewBox="0 0 24 24" fill="white" style="margin-right:8px;">'
    ...
    '<span>jai-rathinavel</span></a></div>')
